...
def test_mediapipe_detector_runs_on_rendered_frame():
    ...
    cannot see. The synthetic box-person is enough for the pose model to fire.
    """
    ...
    assert d.person_mask.any()
    assert len(d.body) == 33
    assert sum(o.valid for o in d.body) >= 15
