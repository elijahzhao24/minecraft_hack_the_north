...
def build_detector(settings: Settings) -> ViewDetector:
    """Select the per-view detector from ``settings.vision_backend``.

    ``mediapipe`` verifies the pinned model files at construction time and fails
    fast with :class:`ModelManifestError` if a file is missing or its hash
    differs; nothing is downloaded. MediaPipe itself is imported lazily on the
    processing thread the first time a frame is detected.
    """
    ...
    if settings.vision_backend == "mediapipe":
        ...
        log_event(
            "info",
            "vision_models_verified",
            pose_sha256=models.pose_sha256[:12],
            hand_sha256=models.hand_sha256[:12],
        )
...
def build_fitter(settings: Settings, calibration: RigCalibration) -> CharacterFitter:
    """Select the character fitter from ``settings.collider_backend``."""
...
def build_debug_hook(
    settings: Settings, calibration: RigCalibration, fitter: CharacterFitter
) -> PostFitHook | None:
    """Return a post-fit hook that writes debug artifacts, or ``None`` if disabled.

    Each processed pair gets its own directory under ``debug_artifacts_dir``
    named by the pair ID; the overlays may show a person, so this directory must
    never be uploaded.
    """
    ...
    def hook(
        ...
        write_debug_artifacts(
            root / str(pair.pair_id),
            frames=frames,
            detections=detections,
            calibrations={d: calibration.camera(d) for d in frames},
            cloud=cloud,
            landmarks=fitted.landmarks,
            colliders=getattr(fitter, "last_typed_colliders", ()),
...
def build_processor(
    ...
    *,
    ...
    """Build a processor; stages come from settings unless injected explicitly."""
